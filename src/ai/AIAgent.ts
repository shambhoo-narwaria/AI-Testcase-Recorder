...
const AIActionSchema = z.object({
  ...
  assertion: z
    .enum(["visible", "notVisible", "textIncludes", "urlIncludes"])
    .nullish()
    .or(z.literal("")),
...
export class AIAgent {
  ...
  constructor(goal: string, maxSteps = 15, config: AIAgentConfig = {}) {
    ...
    this.primaryModel =
      config.primaryModel || process.env.AI_MODEL || inferDefaultModel(baseURL);
    this.modelRotation = dedupeModels([
      this.primaryModel,
      ...(config.fallbackModels?.length
        ? config.fallbackModels
        : parseFallbackModelsFromEnv()),
    ]);
  ...
  async getNextStep(
    ...
    if (domElements.length === 0) {
      console.log(
        "[Agent] No interactive elements found. Waiting for dynamic content."
      );
    ...
    console.log(
      `[Agent] DOM extraction complete. Found ${domElements.length} interactive elements.`
    );
    ...
    while (attempt < 5) {
      try {
        const modelName =
          attempt === 0 ? this.primaryModel : this.getNextModel();
        console.log(
          `[Agent] Sending request to ${this.providerName} (${modelName})...`
        );

        const responseText = await this.requestDecision(
          prompt + schemaErrorFeedback,
          modelName
        );
        console.log(`[Agent] AI Response: ${responseText}`);

        const parsed = AIActionSchema.parse(
          JSON.parse(stripCodeFences(responseText))
        );
        ...
        if (error instanceof SyntaxError || error instanceof z.ZodError) {
          console.log(
            "[Agent] AI returned invalid JSON. Asking it to self-correct."
          );
          schemaErrorFeedback = `\n\nYOUR LAST RESPONSE WAS INVALID. ERROR: ${stringifyError(
            error
        ...
        const status =
          typeof error === "object" && error && "status" in error
            ...
            : undefined;
        console.error(
          `[Agent] API error (${status ?? "unknown"}): ${stringifyError(error)}`
        );
        ...
        if (
          ...
          const nextModel =
            this.modelRotation[
              this.currentModelIndex % this.modelRotation.length
            ];
          console.log(
            `[Agent] Model "${modelName}" failed (status ${status}). Switching to "${nextModel}".`
          );
          attempt += 1;
          continue;
        }
        ...
        const delayMs =
          retryAfterSeconds > 0
            ...
            : 4000 * attempt;
        console.log(
          `[Agent] Waiting ${Math.round(
            delayMs / 1000
          )}s before retry ${attempt}...`
        );
      ...
      previousSteps.length > 0
        ? previousSteps
            ...
            .map((step, index) => {
              ...
              return `${index + 1}. ${
                step.action
              }${targetPart}${valuePart}${resultPart}`;
            })
            .join("\n")
...
`;
  ...
  private async requestDecision(
    ...
    try {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a QA agent. You must only reply with valid JSON.",
            },
      ...
      if (!result.choices?.length) {
        throw new Error(
          `API returned no choices. Response: ${JSON.stringify(result)}`
        );
      }
  ...
  private getNextModel(): string {
    const model =
      this.modelRotation[this.currentModelIndex % this.modelRotation.length];
